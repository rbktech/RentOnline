...
void CMainFrame::Init()
{
    while(m_resolution != false) {
        if(m_client.Connect(IP_CONNECT, 6000) == 0) {
            ...
            m_client.Recv(id, [this](const uint8_t* data, const int& size) { FuncReceiveData(data, size); });
        }
    }
}
...
void CMainFrame::FuncReceiveData(const uint8_t* data, const int& size)
{
    if(FillingMessage(data, size, m_part) == true) {
        ...
        if(parse.size() == 8) {
            ...
            if(CFile::CreateWriteFile(m_person.namePhoto, parse.at(7).first, parse.at(7).second) ==
                FileResult::Success) {
                m_setPerson = true;
...
void CMainFrame::CheckNewAccount(wxCommandEvent& event)
{
    ...
    SendFunc func = [=](int sock, const uint8_t* data, const int& size) { m_client.Send(sock, data, size); };
